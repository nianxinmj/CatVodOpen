...
async function request(reqUrl, agentSp) {
    let res :AxiosResponse<asy> = await req(reqUrl, {
        method: 'get',
        headers: {
...
async function home(_inReq, _outResp) {
    let classes = [{type_neme:string,type_id:... = [{"type_id":1,"type_name":"电影"},{"type_id":2,"type_name":"追剧"},{"type_id":3,"type_name":"综艺"},{"type_id":4,"type_name":"动漫"}];
    let filterObj = {
...
async function category(_inReq, _outResp) {
    ...
    let pg = _inReq.body.page;
    const filters = _inReq.body.filters || {};
    if (pg <= 0) pg = 1;
    const link = url + '/vodshow/' + (filters.cateId || tid) + '-' + (filters.area || '') + '-' + (filters.by || 'time') + '--' + (filters.lang || '') + '----' + pg + '---' + (filters.year || '') + '.html';
